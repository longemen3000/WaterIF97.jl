...
function temperature_impl(mt::SingleSatP,model::IndustrialWater,p)
P = normalize_units(p)
...
#temperature_impl(mt::SingleSatP,model::IndustrialWater,p)
function pressure_impl(mt::SingleSatT,model::IndustrialWater,t)
    T = normalize_units(t)
...
pressure_impl(mt::SingleΦT,model::IndustrialWater,t) = pressure_impl(QuickStates.sat_p(),model,t)
temperature_impl(mt::SingleΦP,model::IndustrialWater,p) = temperature_impl(QuickStates.sat_t(),model,p)

function pressure(model::IndustrialWater,st::ThermodynamicState,unit = u"Pa")
    return pressure(state_type(st),model,st,unit)
end

function pressure(mt::SingleSatT,model::IndustrialWater,st::ThermodynamicState,unit)
    t = temperature(FromState(),st)
    res = pressure_impl(mt,model,t)
    return convert_unit(u"Pa",unit,res)
end

function temperature(model::IndustrialWater,st::ThermodynamicState,unit = u"K")
    return pressure(state_type(st),model,st,unit)
end

function temperature(mt::SingleSatP,model::IndustrialWater,st::ThermodynamicState,unit)
...
function region_id(mt::SinglePT,model::IndustrialWater,p, t)::Symbol
    function Psat(Tx)
...
function region_id(mt::SinglePH,model::IndustrialWater,p, _h)::Symbol
...
function region_id(mt::SinglePS,model::IndustrialWater,p, _s)::Symbol
    function Psat(Tx)
        p = pressure_impl(QuickStates.sat_t(),model,Tx) 
        return convert_unit(u"Pa",u"MPa",p)
    end
